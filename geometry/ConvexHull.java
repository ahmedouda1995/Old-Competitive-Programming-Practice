...
import geometry.GeometricFunctions.Vect;
...
public class ConvexHull {
	...
	public static void main(String[] args) throws IOException {
		...
		for(Point p : ret)
		{
			if(poly1.contains(new Pair(p.x, p.y)))
				first = true;
	...
	{
		return Math.abs((new Vect(p, q)).cross(new Vect(p, r))) < EPS ||
				(new Vect(p, q)).cross(new Vect(p, r)) > 0;
	}
